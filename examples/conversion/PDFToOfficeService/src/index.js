...
router.post('/api/upload', async (ctx) => {
  ...
  try {
    ...
    fs.renameSync(
      path.join(__dirname, 'static/fileUploads/', file.newFilename),
      path.join(fullPath, file.newFilename),
    );
  } catch (e) {
...
router.post('/api/convert', (ctx) => {
  ...
  var range = new Range();
  let setting_data = new PDF2OfficeSettingData(
    path.join(__dirname, 'metrics_data'),
    AIRecognize,
    range,
    true,
    word_setting_data,
